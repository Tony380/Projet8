...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for element in name:
            ...
            cat.save()
            self.stdout.write('Import des produits de la catégorie {}'.format(element))
            payload = {"search_terms": "{}".format(cat), "page_size": 50, "json": 1}
            ...
            for i in products:
                ...
                try:
                    if i.get("product_name", False) and i.get("brands", False) and \
                            i.get("nutrition_grades", False) and i.get("image_front_url", False):
                        product = Product.objects.create(name=i["product_name"],
                                                         brands=i["brands"],
                                                         link=i["url"],
                                                         nutriscore=i["nutrition_grades"],
                                                         image=i["image_front_url"],
                                                         fat=i["nutriments"]["fat_100g"],
                                                         saturated_fat=i["nutriments"]["saturated-fat_100g"],
